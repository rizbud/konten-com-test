...
import { APPROVE_BUTTON, DANGER, MUTED, QUIET_BUTTON, REJECT_BUTTON } from '../ui'
        ...
        <>
          <p>
            {approving ? (
              <>
                ...
                <strong>{submission.campaignTitle}</strong>&rsquo;s remaining budget.
              </>
            ) : (
              <>
                This closes <strong>{submission.creatorUsername}</strong>&rsquo;s
                submission to <strong>{submission.campaignTitle}</strong> without
                paying it. No earning is written and no budget is spent.
              </>
            )}
          </p>
          ...
          {approving ? (
            <dl className={`mt-3 flex gap-6 text-xs ${MUTED}`}>
              <div>
                <dt>Gross</dt>
                <dd className="tabular-nums">{formatRupiah(preview?.gross ?? 0)}</dd>
              </div>
              <div>
                <dt>Platform fee</dt>
                <dd className="tabular-nums">{formatRupiah(preview?.fee ?? 0)}</dd>
              </div>
              <div>
...
export function SubmissionDetailDialog({
  ...
  return (
    <Modal
      ...
      footer={
        <button type="button" onClick={onClose} className={`h-9 ${QUIET_BUTTON}`}>
          Close
        </button>
      }
    >
